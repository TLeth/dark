...
class Culler {
  double tx;
  double ty; // Tangent line
  double td; // Distance to tangent line from origin
  double c;
  // Distance to tangent line from origin
  double s; // cos and sin
  double xc;
  // cos and sin
  double yc; // Center
  ...
  bool isVisible(Bounds b) {
    ...
    List<double> xCorners = [xProject(b.x0, b.y0), xProject(b.x1, b.y0), xProject(b.x1, b.y1), xProject(b.x0, b.y1),];
  ...
  void clipRegion(double x0, double x1) {
    ...
    for (int i = 0; i < clipRangeCount; i++) {
      ...
      if (cr.x0 > x1 || cr.x1 < x0) {
        ...
        if (clipRangeCount-- > 1) clipRanges[i--] = clipRanges[clipRangeCount];
      }
    }
    ...
    for (int i = 0; i < subSector.segs.length; i++) {
      ...
      if (rangeVisible(xp0, xp1)) {
        ...
        if (seg.backSector == null || !seg.wall.data.twoSided) {
          ...
          if (seg.backSector.floorHeight >= seg.backSector.ceilingHeight) shouldClip = true; else if (seg.sector.floorHeight >= seg.backSector.ceilingHeight) shouldClip = true; else if (seg.sector.ceilingHeight <= seg.backSector.floorHeight) shouldClip = true;
...
//  final Vector2 pos;
  ...
  void findSortedSegs(Culler culler, double x, double y, List<Segment> result) {
    if (x * dx + y * dy > d) {
      if (leftChild != null) leftChild.findSortedSegs(culler, x, y, result); else culler.checkOccluders(leftSubSector, result, _subSectorCount++);

      if (culler.isVisible(rightBounds)) {
        if (rightChild != null) rightChild.findSortedSegs(culler, x, y, result); else culler.checkOccluders(rightSubSector, result, _subSectorCount++);
      }
    } else {
      if (rightChild != null) rightChild.findSortedSegs(culler, x, y, result); else culler.checkOccluders(rightSubSector, result, _subSectorCount++);
  ...
  static void findSubSectorsInRadius(BSPNode n, double x, double y, double r, List<SubSector> result) {
    ...
    do {
      ...
      if (dd >= n.d - r) {
        if (n.leftChild != null) next = n.leftChild; else result.add(n.leftSubSector);
      }
      if (dd <= n.d + r) {
  ...
  void findSectorsInRadius(double x, double y, double radius, HashSet<Sector> result) {
    double dd = x * dx + y * dy;
    if (dd > d - radius) {
      if (leftChild != null) leftChild.findSectorsInRadius(x, y, radius, result); else result.add(leftSubSector.sector);
    }
    if (dd < d + radius) {
      if (rightChild != null) rightChild.findSectorsInRadius(x, y, radius, result); else result.add(rightSubSector.sector);
    }
  }

  SubSector findSubSector(double x, double y) {
    if (x * dx + y * dy > d) {
      if (leftChild != null) return leftChild.findSubSector(x, y); else return leftSubSector;
    } else {
      if (rightChild != null) return rightChild.findSubSector(x, y); else return rightSubSector;
    }
  }
  ...
  void getPotentiallyIntersectingSegs(double x0, double y0, double xn, double yn, List<Segment> segments) {
    ...
    if (distance > 0.0) {
      if (leftChild != null) leftChild.getPotentiallyIntersectingSegs(x0, y0, xn, yn, segments); else segments.addAll(leftSubSector.segs);

      if (normalDotProduct < 0.0) {
        if (rightChild != null) rightChild.getPotentiallyIntersectingSegs(x0, y0, xn, yn, segments); else segments.addAll(rightSubSector.segs);
      }
    } else {
      if (rightChild != null) rightChild.getPotentiallyIntersectingSegs(x0, y0, xn, yn, segments); else segments.addAll(rightSubSector.segs);
